...
logger.add("logs/crawler_{time}.log", rotation="10 MB", level="DEBUG")
...
def main():
    ...
    # 每篇帖子最大评论数限制（默认250，设为0或None表示不限制）
    MAX_COMMENTS = 50

    # 向量数据库配置
    ENABLE_VECTOR_STORE = True  # 是否启用向量数据库
    ...
    print(f"向量数据库: {'启用' if ENABLE_VECTOR_STORE else '禁用'}")
...
def advanced_example():
    ...
    crawler = TaogubaCrawler(
        ...
        enable_vector_store=True,  # 启用向量数据库
        vector_store_config={
            "collection_name": "taoguba_posts",
...
def rag_search_example():
    ...
    # 搜索帖子内容
    print("搜索帖子内容（关于'涨停'）:")
    results = vectorizer.search_posts("涨停", top_k=5)
    for doc in results:
        print(f"- [{doc.metadata.get('author_name')}] {doc.content[:100]}...")
        print(f"  相似度: {doc.metadata.get('similarity_score', 'N/A')}")
        print()

    # 搜索评论
    print("\n搜索评论（关于'买入'）:")
    results = vectorizer.search_comments("买入", top_k=5)
    for doc in results:
        print(f"- [{doc.metadata.get('author_name')}] {doc.content[:100]}...")
        print(f"  楼层: {doc.metadata.get('floor_number', 'N/A')}")
        print()

    # 搜索所有内容
    print("\n综合搜索（关于'龙头'）:")
    results = vectorizer.search_all("龙头", top_k=3)
